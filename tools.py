...
def search_flights(origin: str, destination: str) -> str:
    ...
    # Nếu không tìm thấy, thử tra ngược lại
    if not flights:
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    result = f"Danh sách khách sạn tại {city} (Sắp xếp theo đánh giá):\n"
    for h in filtered_hotels:
        price_fmt = "{:,.0f}đ".format(h['price_per_night']).replace(",", ".")
        result += f"- {h['name']} ({h['stars']} sao) - Khu vực: {h['area']} | Đánh giá: {h['rating']} | Giá: {price_fmt}/đêm\n"
    return result
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        for item in items:
            ...
            detail_list += f"- {name.strip()}: {'{:,.0f}đ'.format(amount).replace(',', '.')}\n"
        ...
        if remaining < 0:
            result += f"\nVượt ngân sách {'{:,.0f}đ'.format(abs(remaining)).replace(',', '.')}! Cần điều chỉnh."
            
        return result
    except Exception:
        return "Lỗi: Định dạng chuỗi expenses sai. Vui lòng dùng 'tên:số_tiền,tên:số_tiền'."

# Khai báo danh sách tools để dùng trong Agent
